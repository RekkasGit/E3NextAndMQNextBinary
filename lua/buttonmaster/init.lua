...
local DrawButtonTooltip = function(Button)
    ...
    if Button.Unassigned == nil and ImGui.IsItemHovered() then
        ImGui.BeginTooltip()
        ImGui.Text(tostring(Button.Label))
        ImGui.EndTooltip()
    end
end
...
local RecalculateVisibleButtons = function()
    ...
    local rows = math.floor(lastWindowHeight / (btnSize + 10))
    local cols = math.floor(lastWindowWidth / (btnSize + 7))
    local count = 100
...
local DrawContextMenu = function(Set, Index)
    ...
    if ImGui.BeginPopupContextItem() then
        ...
        if getTableSize(unassigned) > 0 then
            ImGui.SetNextWindowSize(ImVec2(0, 700))
            if ImGui.BeginMenu("Assign Hotkey") then
                ...
                table.sort(sortedKeys, function(a, b)
                    local labelA = unassigned[a] and unassigned[a].Label:lower()
                    local labelB = unassigned[b] and unassigned[b].Label:lower()
...
local HandleEdit = function(Set, Index, Key, Prop)
    local value = type(tmpButton[Key][Prop]) == 'number' and tostring(tmpButton[Key][Prop]) or tmpButton[Key][Prop]
    local txt, selected = ImGui.InputText(Prop, value or '', 0)
    if selected then
...
local DrawButtons = function(Set)
    ...
    local btnSize = (settings['Global']['ButtonSize'] or 6) * 10
    local cols = math.floor(ImGui.GetWindowSize() / (btnSize + 7))
    ...
    for i, ButtonIndex in ipairs(buttons) do
        ...
        if clicked then
            ...
            if ImGui.BeginDragDropSource() then
                ...
                ImGui.Button(tostring(Button.Label), btnSize, btnSize)
...
local LoadSettings = function()
    settings_file = 'ButtonMaster.ini'
    settings_path = mq.configDir .. '/' .. settings_file
